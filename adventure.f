C ADVENTURES
     ...
     1 ,30,0,0,3,3/
      ...
      DATA(DTRAV(I),I=1,20)/36,28,19,30,62,60,41,27,17,15,19,28,36
     1 ,300,300,5*0/
      DATA CHRS /'    '/, SETUP /0/
      DATA KEY /300*0/
      DATA DSEEN /10*0/, DLOC /10*0/, ODLOC /10*0/
      DATA LOCOUT /.FALSE./

C PARSE COMMAND LINE ARGS
      DO I=1,IARGC()
          CALL GETARG(I,A)
          IF (A.EQ.'-LOC' .OR. A.EQ.'-loc') LOCOUT=.TRUE.
      END DO
...
C DWARF STUFF
      ...
      IF (LOCOUT) PRINT '(A,1x,i2)','LOC:',LOC
...
71    KK=STEXT(L)
      ...
      IF(KK.EQ.0) GOTO 7
C4     TYPE 5,(LLINE(KK,JJ),JJ=3,LLINE(KK,2))
C5     FORMAT(20A5)
...
21    IF(L.LT.300)GOTO 2
      IL=L-300+1
C     GOTO(22,23,24,25,26,31,27,28,29,30,33,34,36,37)IL
      GOTO(22,23,24,25,26,31,27,28,29,30,33,34,36,37,39)IL
      PRINT '(a,1x,i2,1x,a,1x,i3,a)',
     *  '**No special motion',IL-1,'from',LOC,'- hope this is OK...'
      GOTO 2
...
C2005  TYPE 2006,(LLINE(KK,JJ),JJ=3,LLINE(KK,2))
C2006  FORMAT(20A5)
...
5506  IF(JOBJ.NE.WATER)JSPK=78
      ...
      KKT=RTEXT(IT)
      IF(KKT.EQ.0)RETURN
C999   TYPE 998, (LLINE(KKT,JJT),JJT=3,LLINE(KKT,2))
C998   FORMAT(20A5)
...
996   FORMAT(/)
      ...
      DIMENSION A(5),M2(6)
C     DATA M2/"4000000000,"20000000,"100000,"400,"2,0/
C6     ACCEPT 1,(A(I), I=1,4)
C1     FORMAT(4A5)
      READ(*,1) LINE
1     FORMAT(A)
      DO 5 J=1,LEN(LINE)
      IF(LINE(J:J).GE.'a'.AND.LINE(J:J).LE.'z')
     1  LINE(J:J)=CHAR(ICHAR(LINE(J:J))-ICHAR('a')+ICHAR('A'))
5     CONTINUE
      TWOW=0
C     S=0
C     B=A(1)
C     DO 2 J=1,4
C     DO 2 K=1,5
C     MASK1="774000000000
C     IF(K.NE.1) MASK1="177*M2(K)
C     IF(((A(J).XOR."201004020100).AND.MASK1).EQ.0)GOTO 3
C     IF(S.EQ.0) GOTO 2
C     TWOW=1
C     CALL SHIFT(A(J),7*(K-1),XX)
C     CALL SHIFT(A(J+1),7*(K-6),YY)
C     MASK=-M2(6-K)
C     C=(XX.AND.MASK)+(YY.AND.(-2-MASK))
C     GOTO 4
C3     IF(S.EQ.1) GOTO 2
C     S=1
C     IF(J.EQ.1) B=(B.AND.-M2(K)).OR.("201004020100.AND.
C    1 (-M2(K).XOR.-1))
C2     CONTINUE
C4     D=A(2)
      B=LINE(1:5)
...
C     SHIFT VAL BY |DIST| BITS RIGHT (IF DIST < 0) OR LEFT (IF DIST > 0)
      IMPLICIT INTEGER (A-Z)
      DATA MR1 /Z'7FFF FFFF'/
      DATA ML1 /Z'4000 0000'/
      DATA ML2 /Z'3FFF FFFF'/
      DATA MSG /Z'8000 0000'/
      RES=VAL
      IF(DIST)10,20,30
10    IDIST=-DIST
      DO I=1,IDIST
      J = 0
      IF (RES.LT.0) J=MSG
C11    RES = ((RES.AND."377777777777)/2) + J
      RES = IOR(IAND(RES,MR1)/2,J)
      END DO
20    RETURN
30    DO I=1,DIST
      J = 0
C     IF ((RES.AND."200000000000).NE.0) J="400000000000
      IF (IAND(RES,ML1).NE.0) J=MSG
C31    RES = (RES.AND."177777777777)*2 + J
